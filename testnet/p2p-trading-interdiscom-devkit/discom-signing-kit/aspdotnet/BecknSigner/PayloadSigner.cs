...
public record SignerConfig
{
    ...
    public required string UniqueKeyId { get; init; }
    ...
    public TimeSpan ExpiryDuration { get; init; } = TimeSpan.FromMinutes(5);
}
...
public class PayloadSigner
{
    ...
    {
        ArgumentException.ThrowIfNullOrEmpty(config.SubscriberId, nameof(config.SubscriberId));
        ArgumentException.ThrowIfNullOrEmpty(config.UniqueKeyId, nameof(config.UniqueKeyId));
        ArgumentException.ThrowIfNullOrEmpty(config.SigningPrivateKey, nameof(config.SigningPrivateKey));
